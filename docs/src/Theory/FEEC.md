...
Assume that we are interested in numerical methods for mixed weak forms of scalar/vector Laplacians on domains $\Omega \subset \mathbb{R}^3$.
Then, the Sobolev spaces that appear in the discretizations of such weak forms can be combined into a sequence that connects these spaces using the differential operators that appear in the weak forms:
...
For the purpose of our discussion, let us assume that $\Omega$ is (topologically) a solid ball, so that $\dim(\mathcal{H}^0) = 1$, $\dim(\mathcal{H}^1) = 0 = \dim(\mathcal{H}^2) = \dim(\mathcal{H}^3) = 0$. In this case, the complex is said to be **exact**.
...
Using this notation, and since we are working with an exact complex, it can be shown that the following (mixed) weak problem is well-posed for all $i \in \{0, 1, 2, 3\}$: find $(\sigma, u, p) \in V^{i-1} \times V^{i} \times \mathcal{H}^i$ such that
...
For instance, when $i = 0$, we have the standard Poisson problem discussed in the previous section (albeit now with homogeneous natural boundary conditions): find $(u, p) \in H^1(\Omega) \times \mathbb{R}$ such that
