...
x_n = TimeIntegrators.initializeScheme(x, method)
v_n = TimeIntegrators.initializeScheme(v, method)
...
fps = 30
function animate_solution(xs, ys, x_n, v_n, T, t0, dt, fps, ode_velocity, ode_position, trail_length = 30)
    N_t = round(Int, T / dt)
    ...
    for i = 1:N_t
        t = t0 + dt * i
        TimeIntegrators.timeIntegrate!(v_n, ode_velocity, t, dt; pos=TimeIntegrators.get_solution(x_n))
        TimeIntegrators.timeIntegrate!(x_n, ode_position, t+dt/2, dt; vel=TimeIntegrators.get_solution(v_n))
